...
class OSMN_NET(nn.Module):
    ...
    def __init__(self,mod_early_conv=False):
        ...
        self.vg_features = nn.Sequential(
            ...
            # no BN for these layers, since batchsize can't be 1 when apply feature wise BN
        ...
        self.conv1_att = ConvBN(1, 64 * self.num_mod_layers[0], kernel_size=1)
        self.conv2_att = ConvBN(1, 128 * self.num_mod_layers[1], kernel_size=1)
        self.conv3_att = ConvBN(1, 256 * self.num_mod_layers[2], kernel_size=1)
        self.conv4_att = ConvBN(1, 512 * self.num_mod_layers[3], kernel_size=1)
        self.conv5_att = ConvBN(1, 512 * self.num_mod_layers[4], kernel_size=1)
    ...
    def forward(self, input_image,vg_image,gb_image):
        ...
        seg_1 = self.seg_block1(input_image, md_p1, conv1_att)

        seg_2 = F.max_pool2d(seg_1,2,2)
        seg_2 = self.seg_block2(seg_2, md_p2, conv2_att)

        seg_3 = F.max_pool2d(seg_2, 2, 2)
        seg_3 = self.seg_block3(seg_3, md_p3, conv3_att)

        seg_4 = F.max_pool2d(seg_3, 2, 2)
        seg_4 = self.seg_block4(seg_4, md_p4, conv4_att)

        seg_5 = F.max_pool2d(seg_4, 2, 2)
        seg_5 = self.seg_block5(seg_5, md_p5, conv5_att)
...
class modulated_conv_block(nn.Module):
    def __init__(self,blocks,vm=False,sm=False):
        ...
        self.conv1 = nn.Sequential(
            blocks[0],
            nn.BatchNorm2d(blocks[0].out_channels),
            nn.ReLU(inplace=True)
        )
        self.conv2 = nn.Sequential(
            blocks[1],
            nn.BatchNorm2d(blocks[1].out_channels),
            nn.ReLU(inplace=True)
        )
        if self.block_num>2:
            self.conv3 = nn.Sequential(
                blocks[2],
                nn.BatchNorm2d(blocks[2].out_channels),
                nn.ReLU(inplace=True)
            )
...
class ConvBN(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,kernel_size,padding=0,if_active=True):
